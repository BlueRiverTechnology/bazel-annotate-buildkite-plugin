...
process_bep() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$line" == *'"buildStarted"'* ]]; then
      build_start_time=$(jq -r '.buildStarted.startTimeMillis // 0' <<<"$line")
      continue
    elif [[ "$line" == *'"buildFinished"'* ]]; then
      build_end_time=$(jq -r '.buildFinished.finishTimeMillis // 0' <<<"$line")
      continue
    fi

    if [[ "$line" == *'"targetCompleted"'* ]]; then
      local label=$(jq -r '.id.targetCompleted.label // "unknown"' <<<"$line")
      local success=$(jq -r '.completed.success // "false"' <<<"$line")

      if jq -e '.completed.outputGroup?[]?.fileSets?[]?.id? != null' <<<"$line" >/dev/null 2>&1 && \
         ! jq -e '.completed.actionExecuted? != null' <<<"$line" >/dev/null 2>&1; then
        ((cached_count++))
      fi
    ...
    if [[ "$line" == *'"testResult"'* ]]; then
      ...
      if [[ "$test_status" != "PASSED" ]]; then
        ...
        jq -e '.testResult.testActionOutput? != null' <<<"$line" >/dev/null 2>&1 && \
        ...
        if jq -e '.testResult.testActionOutput[]? | select(.name == "test.log")' <<<"$line" >/dev/null 2>&1; then
  ...
  [[ -n "$failure_details" ]] && summary+=$'<details open><summary><strong>❌ Failure Details</strong></summary>\n\n'"$failure_details"$'\n</details>\n\n'
